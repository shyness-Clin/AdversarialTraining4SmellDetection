...
def embedding_lstm_with_pretrained_weights(data, config, weight_file_path, smell, out_folder=OUT_FOLDER, dim=DIM):
    tf.keras.backend.clear_session()
    max_features = int(max(np.max(data.train_data), np.max(data.eval_data)))
    print("max features: " + str(max_features))
    model = tf.keras.models.Sequential()
...
if __name__ == "__main__":
    ...
    data = get_all_data(data_path, smell)
    weight_file_path = "weights_best.rnn.ComplexMethod1.hdf5"
    config = configuration.RNN_emb_lstm_config(
        emb_output=32,
        lstm_units=128,
        layers=1,
    ...
    embedding_lstm_with_pretrained_weights(data, config, weight_file_path, smell)
